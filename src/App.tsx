        ...
        const SundaySchoolApp = () => {
            ...
            useEffect(() => {
                ...
                const loadLessons = useCallback(async () => {
                    // Prevent double-loading which often hangs mobile devices
                    if (lessonsLoadingRef.current) return;
                    lessonsLoadingRef.current = true;
                    setLessonsLoading(true);
                    ...
                    try {
                        ...
                        // 1. Update the full list first so the dropdown has data
                        setLessons(rows);

                        if (rows.length > 0) {
                            // 2. Look for the lesson marked active in the DB
                            const active = rows.find(l => l.is_active) || rows[0];
                            
                            // 3. Force the app to focus on this lesson
                            setActiveLesson(active.id);
                            setContentData(hydrateLessonData(active.content));
                        }
                    } catch (err) {
